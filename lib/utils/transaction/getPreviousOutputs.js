...
async function getPreviousOutputs({ wallets, database, queryParams }) {
  let addresses = Array.from(wallets.keys())
    .map(address => ({address, countspaces: wallets.get(address).countspaces}))
  let pointers = (await getPreviousPointers({addresses, database, queryParams}))
...
async function getPreviousPointers({ addresses, database, queryParams }) {
